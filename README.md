...
| `--html` | PDF ではなく HTML を出力して開く（出力先の拡張子を .html に変えて保存） |
| `--no-open` | 生成後に PDF（または HTML）を開かない |
...
セキュリティ製品などの影響で Edge も Chrome もヘッドレス実行できない環境では、PDF を生成できません。その場合は自動で HTML 出力に切り替わり、出力先と同じ場所に .html を保存して既定のブラウザーで開きます。表示されたページで Ctrl+P →「PDF として保存」を選べば PDF にできます。最初から HTML を出力するには `--html` を指定します。
